...
struct diffproc_transport {
  diffproc_transport(const grpc_transport_vtable* vtable, shared_mu* mu,
                   bool is_client)
      : mu(mu),
        is_client(is_client),
        state_tracker(is_client ? "diffproc_client" : "diffproc_server",
                      GRPC_CHANNEL_READY) {
    base.vtable = vtable;
  ...
  struct diffproc_transport* other_side;
  struct diffproc_stream* stream_list = nullptr;
};

struct diffproc_stream {
  diffproc_stream(diffproc_transport* t, grpc_stream_refcount* refcount,
                const void* server_data, grpc_core::Arena* arena)
      : t(t), refs(refcount), arena(arena) {
    // Ref this stream right now for ctor and list.
    ref("diffproc_init_stream:init");
    ref("diffproc_init_stream:list");
    ...
    if (!server_data) {
      t->ref();
      diffproc_transport* st = t->other_side;
      st->ref();
      other_side = nullptr;  // will get filled in soon
      // Pass the client-side stream address to the server-side for a ref
      ref("diffproc_init_stream:clt");  // ref it now on behalf of server
                                      // side to avoid destruction
      diffproc_LOG(GPR_INFO, "calling accept stream cb %p %p",
                 st->accept_stream_cb, st->accept_stream_data);
      (*st->accept_stream_cb)(st->accept_stream_data, &st->base, (void*)this);
    } else {
      // This is the server-side and is being called through accept_stream_cb
      diffproc_stream* cs = (diffproc_stream*)server_data;
      other_side = cs;
      // Ref the server-side stream on behalf of the client now
      ref("diffproc_init_stream:srv");
...
#define STREAM_UNREF(refs, reason) grpc_stream_unref(refs)
  ...
  void unref(const char* reason) {
    diffproc_LOG(GPR_INFO, "unref_stream %p %s", this, reason);
    STREAM_UNREF(refs, reason);
  }
#undef STREAM_REF
#undef STREAM_UNREF

  diffproc_transport* t;
  ...
  struct diffproc_stream* other_side;
  ...
  bool listed = true;
  struct diffproc_stream* stream_list_prev;
  struct diffproc_stream* stream_list_next;
};
...
void log_metadata(const grpc_metadata_batch* md_batch, bool is_client,
  ...
  for (grpc_linked_mdelem* md = md_batch->list.head; md != nullptr;
    ...
    gpr_log(GPR_INFO, "diffproc:%s:%s: %s: %s", is_initial ? "HDR" : "TRL",
...
int init_stream(grpc_transport* gt, grpc_stream* gs,
                ...
                grpc_core::Arena* arena) {
  diffproc_LOG(GPR_INFO, "init_stream %p %p %p", gt, gs, server_data);
  diffproc_transport* t = reinterpret_cast<diffproc_transport*>(gt);
  new (gs) diffproc_stream(t, refcount, server_data, arena);
  return 0;  // return value is not important
}

void close_stream_locked(diffproc_stream* s) {
  if (!s->closed) {
    ...
    if (s->listed) {
      diffproc_stream* p = s->stream_list_prev;
      diffproc_stream* n = s->stream_list_next;
      if (p != nullptr) {
...
void close_other_side_locked(diffproc_stream* s, const char* reason) {
...
void complete_if_batch_end_locked(diffproc_stream* s, grpc_error* error,
  ...
  if ((is_sm + is_stm + is_rim + is_rm + is_rtm) == 1) {
    diffproc_LOG(GPR_INFO, "%s %p %p %p", msg, s, op, error);
...
void fail_helper_locked(diffproc_stream* s, grpc_error* error) {
  ...
  if (!s->trailing_md_sent) {
    ...
    diffproc_stream* other = s->other_side;
  ...
  if (s->recv_trailing_md_op) {
    diffproc_LOG(GPR_INFO, "fail_helper %p scheduling trailing-metadata-ready %p",
               s, error);
    grpc_core::ExecCtx::Run(
        DEBUG_LOCATION,
        ...
        GRPC_ERROR_REF(error));
    diffproc_LOG(GPR_INFO, "fail_helper %p scheduling trailing-md-on-complete %p",
               s, error);
    complete_if_batch_end_locked(
...
// true in the future.
void message_transfer_locked(diffproc_stream* sender, diffproc_stream* receiver) {
  size_t remaining =
  ...
  diffproc_LOG(GPR_INFO, "message_transfer_locked %p scheduling message-ready",
             receiver);
  grpc_core::ExecCtx::Run(
      DEBUG_LOCATION,
...
void op_state_machine_locked(diffproc_stream* s, grpc_error* error) {
  ...
  diffproc_LOG(GPR_INFO, "op_state_machine %p", s);
  ...
  if (s->send_trailing_md_op &&
    ...
    if (*destfilled || s->trailing_md_sent) {
      ...
      diffproc_LOG(GPR_INFO, "Extra trailing metadata %p", s);
  ...
  if (s->to_read_trailing_md_filled) {
    ...
    if (s->recv_message_op != nullptr) {
      ...
      *s->recv_message_op->payload->recv_message.recv_message = nullptr;
      diffproc_LOG(GPR_INFO, "op_state_machine %p scheduling message-ready", s);
      grpc_core::ExecCtx::Run(
          DEBUG_LOCATION,
          s->recv_message_op->payload->recv_message.recv_message_ready,
          GRPC_ERROR_NONE);
...
bool cancel_stream_locked(diffproc_stream* s, grpc_error* error) {
  ...
  diffproc_LOG(GPR_INFO, "cancel_stream %p with %s", s, grpc_error_string(error));
  if (s->cancel_self_error == GRPC_ERROR_NONE) {
    ret = true;
...
void perform_stream_op(grpc_transport* gt, grpc_stream* gs,
                       grpc_transport_stream_op_batch* op) {
  diffproc_LOG(GPR_INFO, "perform_stream_op %p %p %p", gt, gs, op);
  diffproc_stream* s = reinterpret_cast<diffproc_stream*>(gs);
  ...
  if (op->cancel_stream) {
    ...
    diffproc_LOG(GPR_INFO, "perform_stream_op %p %s%s%s%s%s%s%s", s,
  ...
  if (error == GRPC_ERROR_NONE &&
    ...
    if (error == GRPC_ERROR_NONE && op->send_initial_metadata) {
      ...
      if (*destfilled || s->initial_md_sent) {
        // The buffer is already in use; that's an error!
        diffproc_LOG(GPR_INFO, "Extra initial metadata %p", s);
  ...
  if (error == GRPC_ERROR_NONE &&
    ...
    diffproc_LOG(GPR_INFO, "perform_stream_op %p scheduling on_complete %p", s,
               error);
...
void close_transport_locked(diffproc_transport* t) {
  diffproc_LOG(GPR_INFO, "close_transport %p %d", t, t->is_closed);
...
void destroy_stream(grpc_transport* /*gt*/, grpc_stream* gs,
  ...
  diffproc_LOG(GPR_INFO, "destroy_stream %p %p", gs, then_schedule_closure);
  diffproc_stream* s = reinterpret_cast<diffproc_stream*>(gs);
...
void destroy_transport(grpc_transport* gt) {
  diffproc_transport* t = reinterpret_cast<diffproc_transport*>(gt);
  diffproc_LOG(GPR_INFO, "destroy_transport %p", t);
  gpr_mu_lock(&t->mu->mu);
  close_transport_locked(t);
  gpr_mu_unlock(&t->mu->mu);
  t->other_side->unref();
...
grpc_endpoint* get_endpoint(grpc_transport* t) {
  return (reinterpret_cast<diffproc_transport*>(t))->ep;
}

const grpc_transport_vtable diffproc_vtable = {
    sizeof(diffproc_stream), "diffproc",        init_stream,
    set_pollset,           set_pollset_set, perform_stream_op,
    perform_transport_op,  destroy_stream,  destroy_transport,
    get_endpoint};

/*******************************************************************************
 * Main diffproc transport functions
 */
grpc_transport* diffproc_transport_create(const grpc_channel_args* args,
                                          bool is_client) {
  printf("\n%d :: %s :: %s\n", __LINE__, __func__, __FILE__);
  diffproc_LOG(GPR_INFO, "diffproc_transports_create");
  shared_mu* mu = new (gpr_malloc(sizeof(*mu))) shared_mu();
  diffproc_transport* t = new (gpr_malloc(sizeof(*t))) diffproc_transport(&diffproc_vtable, mu, is_client);
  return nullptr;
}
}  // namespace
...
grpc_transport* diffproc_transport_create(const grpc_channel_args* args,
  ...
  diffproc_LOG(GPR_INFO, "diffproc_transports_create");
  shared_mu* mu = new (gpr_malloc(sizeof(*mu))) shared_mu();
  diffproc_transport* t = new (gpr_malloc(sizeof(*t)))
      diffproc_transport(&diffproc_vtable, mu, is_client);
  return &t->base;
}
/*******************************************************************************
...
void grpc_diffproc_channel_create(grpc_server* server,
                                         grpc_channel_args* args,
                                         void* /*reserved*/) {
  printf("\n%d :: %s :: %s\n", __LINE__, __func__, __FILE__);
  GRPC_API_TRACE("grpc_diffproc_channel_create(server=%p, args=%p)", 2,
                 (server, args));
  printf("\n%d :: %s :: %s\n", __LINE__, __func__, __FILE__);
  grpc_core::ExecCtx exec_ctx;

  // Remove max_connection_idle and max_connection_age channel arguments since
  // those do not apply to diffproc transports.
  ...
  //diffproc_transport_create(&client_transport, args,1);
  // TODO(ncteisen): design and support channelz GetSocket for diffproc.
  /*grpc_server_setup_transport(server, server_transport, nullptr, server_args,
                              nullptr);*/
  /*grpc_channel* channel = grpc_channel_create(
      "diffproc", client_args, GRPC_CLIENT_DIRECT_CHANNEL, client_transport);*/

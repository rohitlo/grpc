  ...
  if (np->shutting_down) {
    grpc_core::ExecCtx::Run(
        ...
        GRPC_ERROR_CREATE_REFERENCING_FROM_STATIC_STRING(
            "Namedpipe is shutting down", &np->shutdown_error, 1));
    return;
  }
...
static void win_write(grpc_endpoint* ep, grpc_slice_buffer* slices,
  ...
  for (int i = 0; i < np->write_slices->count; i++) {
    status = WriteFile(handle,                          // pipe handle
                       ...
                       NULL);
    if(status == 1) puts("Successfully wrote to server end pipe handle....");
    else {
  ...
  //status = WriteFile(handle,  // pipe handle
  //                     buffers,    // message
  //                   (DWORD)np->write_slices->count,  // message length
  //                     &cbWritten,    // bytes written
  //                     NULL);
  //if (!status) {
  //  _tprintf(TEXT("WriteFile to pipe failed. GLE=%d\n"), GetLastError());

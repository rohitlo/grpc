...
namespace {
class NamedPipeChannelCredentialsImpl final : public ChannelCredentials {
 public:
  ...
  std::shared_ptr<Channel> CreateChannelWithInterceptors(
    ...
    grpc_channel* ch =
        grpc_np_channel_create(target.c_str(), &channel_args, nullptr);
    puts("Here");
...
std::shared_ptr<ChannelCredentials> NamedPipeChannelCredentials() {
  return std::shared_ptr<ChannelCredentials>(
      new NamedPipeChannelCredentialsImpl());
}
